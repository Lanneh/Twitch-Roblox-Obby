...
// Each serverId gets exactly one connection at a time
const activeServers = {};
...
async function connectTwitch(username, serverId) {
    ...
    while (attempts < 3) {
        try {
            ...
            console.log(`[Twitch] Connected to channel: ${username} (server ${serverId})`);
            return client;
        } catch (err) {
            attempts++;
            console.error(`[Twitch] Failed to connect (attempt ${attempts}):`, err);
...
// --- 🧩 Register (create or override existing connection)
app.post("/register", async (req, res) => {
    ...
    // If this server already has a client, replace it safely
    if (activeServers[serverId]) {
        const existing = activeServers[serverId];
        console.log(`[Twitch] Replacing connection for server ${serverId} (old user: ${existing.username})`);
        try {
            await existing.client.disconnect();
        } catch (e) {
            console.warn(`[Twitch] Failed to cleanly disconnect old client for ${serverId}:`, e);
        }
    ...
    try {
        ...
        console.error(`[Twitch] Error registering ${serverId}:`, err);
...
app.get("/getMessages", (req, res) => {
    ...
    const messages = activeServers[serverId].messages.splice(0);
    res.json(messages);
});

// --- ❌ Unregister server (stop Twitch connection)
...
app.listen(PORT, () =>
    console.log(`✅ Twitch relay backend running on port ${PORT}`)
);
